      ...
      <main className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30">
        ...
        <section className="max-w-7xl mx-auto px-6 -mt-12 relative z-20 mb-12">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 shadow-2xl border border-white/50">
            <div className="grid grid-cols-3 gap-4">
              ...
              >
                <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl w-14 h-14 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
        ...
        <section className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-4xl font-black text-gray-900">À la une</h2>
        ...
        {/* Section Annonces - Format liste compacte */}
        <section className="relative overflow-hidden py-16">
          {/* Background animé comme page QR code */}
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-4xl font-black text-white mb-2">Opportunités</h2>
                <p className="text-blue-200">Les dernières annonces de la communauté</p>
              </div>
              {recentAnnouncements.length > 0 && (
                <Link
                  href="/annonces"
                  className="group inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white text-gray-900 hover:bg-blue-50 transition-all duration-300 font-semibold shadow-lg"
                >
                  Voir tout
                  <svg className="w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              ...
              <div className="space-y-4">
                {recentAnnouncements.slice(0, 5).map((announcement) => {
                  const typeColors: { [key: string]: string } = {
                    ...
                    collaboration: 'bg-orange-500/20 text-orange-300 border-orange-500/30',
                  ...
                  return (
                    <Link
                      ...
                      className="group bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 hover:bg-white/20 hover:border-white/40 transition-all duration-300 flex items-center gap-6"
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${typeColors[announcement.type] || typeColors.other}`}>
                            {typeLabels[announcement.type] || 'Autre'}
                          </span>
                          {announcement.company && (
                            <span className="text-blue-200 font-medium">{announcement.company}</span>
                          )}
                        </div>
                      ...
                      <svg className="w-6 h-6 text-white/50 group-hover:text-white group-hover:translate-x-2 transition-all flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                      </svg>
                    </Link>
                  )
