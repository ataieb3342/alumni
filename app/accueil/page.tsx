...
import { recentPostsQuery, recentAnnouncementsQuery, popularTestimonialsQuery, recentMembersQuery } from '@/sanity/lib/queries'
...
import DirectoryCard from '@/app/components/DirectoryCard'
...
export default async function AccueilPage() {
  ...
  // Récupérer les 6 derniers articles, annonces, témoignages et nouveaux membres
  const [recentPosts, recentAnnouncements, popularTestimonials, recentMembers] = await Promise.all([
    ...
    client.fetch(recentMembersQuery)
  ])

  // Déterminer l'ordre d'affichage et le style des sections
  const visibleSections = [
    recentPosts.length > 0 ? 'posts' : null,
    recentMembers.length > 0 ? 'members' : null,
    recentAnnouncements.length > 0 ? 'announcements' : null,
    popularTestimonials.length > 0 ? 'testimonials' : null
  ].filter(Boolean)

  // Fonction pour savoir si une section doit être sombre
  const isDarkSection = (sectionId: string) => {
    const index = visibleSections.indexOf(sectionId)
    return index % 2 === 1 // index 0=clair, 1=sombre, 2=clair, 3=sombre
  }
    ...
    <>
      ...
      <main className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30">
        ...
        {recentPosts.length > 0 && (
          <section className={isDarkSection('posts') ? "relative overflow-hidden py-16" : "max-w-7xl mx-auto px-6 py-16"}>
            {isDarkSection('posts') && (
              <div className="absolute inset-0 bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900">
                ...
                <div className="absolute bottom-20 right-1/4 w-64 h-64 bg-indigo-400/5 rounded-full blur-3xl"></div>
              </div>
            )}
        ...
        {recentMembers.length > 0 && (
          <section className={isDarkSection('members') ? "relative overflow-hidden py-16" : "max-w-7xl mx-auto px-6 py-16"}>
            {isDarkSection('members') && (
              <div className="absolute inset-0 bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900">
                <div className="absolute -top-40 -right-40 w-80 h-80 bg-white/5 rounded-full blur-3xl"></div>
                <div className="absolute top-1/2 -left-40 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl"></div>
                <div className="absolute bottom-20 right-1/4 w-64 h-64 bg-indigo-400/5 rounded-full blur-3xl"></div>
            ...
            <div className={isDarkSection('members') ? "relative z-10 max-w-7xl mx-auto px-6" : ""}>
              ...
              <DirectoryCard users={recentMembers} showNewBadge={true} />
            </div>
          </section>
        )}
        ...
        {recentAnnouncements.length > 0 && (
          <section className={isDarkSection('announcements') ? "relative overflow-hidden py-16" : "max-w-7xl mx-auto px-6 py-16"}>
            {isDarkSection('announcements') && (
              <div className="absolute inset-0 bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900">
                <div className="absolute -top-40 -right-40 w-80 h-80 bg-white/5 rounded-full blur-3xl"></div>
                <div className="absolute top-1/2 -left-40 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl"></div>
                <div className="absolute bottom-20 right-1/4 w-64 h-64 bg-indigo-400/5 rounded-full blur-3xl"></div>
        ...
        {popularTestimonials.length > 0 && (
          <section className={isDarkSection('testimonials') ? "relative overflow-hidden py-16" : "max-w-7xl mx-auto px-6 py-16"}>
            {isDarkSection('testimonials') && (
              <div className="absolute inset-0 bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900">
                <div className="absolute -top-40 -right-40 w-80 h-80 bg-white/5 rounded-full blur-3xl"></div>
                <div className="absolute top-1/2 -left-40 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl"></div>
                <div className="absolute bottom-20 right-1/4 w-64 h-64 bg-indigo-400/5 rounded-full blur-3xl"></div>
